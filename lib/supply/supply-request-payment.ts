...
type SupplyRequestQuote = {
  quoteAmountMinor: number;
  quoteCurrency: string;
};

export async function createSupplyRequestPaymentIntent(input: {
...
export function assertSupplyRequestPaymentIntent(
  paymentIntent: Stripe.PaymentIntent,
  quote: SupplyRequestQuote
) {
  ...
  if (paymentIntent.metadata.source !== "supply_request") {
    throw new StripeCheckoutPaymentError(
      "PAYMENT_INVALID",
      "Invalid payment for this supply request.",
      400
    );
  }
}
