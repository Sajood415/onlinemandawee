...
export function buildVendorReviewStatusEmailHtml(input: {
  appName?: string;
  heading: string;
  message: string;
  note?: string;
...
<head>
  ...
  <title>${escapeHtml(app)} — vendor application update</title>
</head>
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;background:${CARD};border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(15,52,96,0.12);">
          <tr>
            <td style="background:linear-gradient(135deg,${NAVY} 0%,#0a2847 100%);padding:22px 24px;text-align:center;">
              ...
              <p style="margin:10px 0 0;font-size:20px;font-weight:800;color:#fff;line-height:1.25;">Vendor application update</p>
            </td>
          </tr>
          <tr>
            <td style="padding:28px 24px 8px;">
              <div style="text-align:center;margin:8px 0 20px;padding:20px 16px;background:${BG};border-radius:12px;border:1px solid #e2e8f0;border-top:4px solid ${PRIMARY};">
          ...
          <tr>
            <td style="padding:0 24px 24px;">
              <p style="margin:0;padding-top:20px;border-top:1px solid #e2e8f0;font-size:12px;line-height:1.5;color:${MUTED};text-align:center;">This is an automated update from ${escapeHtml(app)}.</p>
            </td>
          </tr>
        </table>
      </td>
