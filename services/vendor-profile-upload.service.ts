...
export class VendorProfileUploadService {
  ...
  async uploadLogo(
    ...
    if (vendor.status !== "ACTIVE") {
      throw new AppError({
        ...
        message: "Only active vendors can upload a logo",
        statusCode: 403,
      });
    }
    ...
    const folder = `mandawee/vendors/${vendor.id}/logo`;
      ...
      (resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream(
          ...
          (err, res) => {
            ...
            if (!res?.secure_url || !res.public_id) { reject(new Error("Upload failed")); return; }
