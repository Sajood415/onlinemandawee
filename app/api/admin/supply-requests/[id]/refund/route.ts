...
export const POST = withErrorHandling(
  withRbac(["ADMIN"], async (request, context) => {
    ...
    const input = await parseBody(request, refundSupplyRequestSchema);
    const result = await supplyRequestService.refundForAdmin(params.id, input);
