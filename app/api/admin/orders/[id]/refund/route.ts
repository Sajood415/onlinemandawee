...
import { adminOrderRefundSchema } from "@/validators/refund.validator";
...
export const POST = withErrorHandling(
  withRbac(["ADMIN"], async (request, context) => {
    ...
    const input = await parseBody(request, adminOrderRefundSchema);
    const result = await refundService.adminMarkOrderRefunded(
      context.auth,
      params.id,
      input
    );
